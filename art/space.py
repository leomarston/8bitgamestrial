...
# round glass dome ring (drawn over the pilot's head)
DOME = spr("""
.....KKKKK.....
...KKWWWWWKK...
..KWCCCCCCCWK..
...
def face(img, name, cx, cy):
    """draw the pilot's head (top of the fighter sprite) centred in the dome"""
    rows = fgrid(getattr(C, name)); rw = len(rows[0]); hh = min(9, len(rows))
    px = img.load()
    for ry in range(hh):
        for rx in range(rw):
...
def shuttle(img, name, cx, topY, flip=False, dmg=0):
    """compose a pod: pilot head, glass dome, hull, then damage cracks."""
    w = len(SHUTTLE[0])
    domeCx, domeCy = cx, topY + 12
    face(img, name, domeCx, domeCy + 1)
    blit(img, DOME, domeCx - len(DOME[0]) // 2, topY + 5)
...
def _cracks(img, cx, topY, lvl):
    ...
    seg1 = [(6, 18), (7, 19), (6, 20), (8, 20), (7, 21), (13, 17), (14, 18), (13, 19)]
    seg2 = [(5, 22), (9, 23), (10, 24), (4, 24), (15, 22), (12, 24), (8, 17), (11, 19), (6, 25)]
    ...
    if lvl == 2:                                       # scorch + missing chunk
        for (dx, dy) in [(3, 23), (16, 21), (4, 25), (15, 24)]:
            sp(px, img.width, img.height, ox + dx, topY + dy, col("K"))
