...
(() => {
  ...
  const SOLID = "GDTSCW";
  const grid = Array.from({ length: HT }, () => Array(LOOP).fill("."));
  const trees = [], towers = [];

  function rng(i) { const v = Math.sin(i * 12.9898 + 7.13) * 43758.5453; return v - Math.floor(v); }
  (function genLevel() {
    let x = 0;
  ...
  function mk(name, x, color) { return { name, color, x, y: GROW * TS - PH, vx: 0, vy: 0, onG: true, alive: true, face: 1, coy: 0, walkT: 0, dist: 0 }; }
  function reset() {
  ...
  // ---------- input ----------
  const held = {};
  function jump(p) { if (p.alive && (phase === "play") && (p.onG || p.coy > 0)) { p.vy = JUMP; p.onG = false; p.coy = 0; } }
  ...
  function moveY(p, dy) {
    ...
    if (dy > 0) { const ty = Math.floor((p.y + PH - 1) / TS); for (let tx = l; tx <= r; tx++) if (solidAt(tx, ty)) { p.y = ty * TS - PH; p.vy = 0; p.onG = true; break; } }
  ...
  function update(dt) {
    ...
    for (const [p, L, R] of [[p1, "KeyA", "KeyD"], [p2, "ArrowLeft", "ArrowRight"]]) {
      ...
      moveX(p, p.vx * dt);
      const wasG = p.onG; moveY(p, p.vy * dt);
      if (p.onG) p.coy = 0.09; else p.coy = Math.max(0, p.coy - dt);
      if (Math.abs(p.vx) > 1 && p.onG) p.walkT += dt * 12;
      // right edge clamp (can't leave the front of the frame)
      if (p.x > cam + viewW - PW) p.x = cam + viewW - PW;
      // death: behind the frame, or fell in a pit
      if (p.x + PW < cam + 2 || p.y > HT * TS + 6) { p.alive = false; p.deathX = p.x; }
      p.dist = p.x;
    }
    const alive = players.filter(p => p.alive);
    if (alive.length <= 1) {
      if (alive.length === 1) winner = alive[0];
      else winner = p1.x >= p2.x ? p1 : p2;   // both fell same frame -> further wins
      phase = "over";
    }
  }
  ...
  function drawPlayer(p) {
    ...
    const bob = (p.onG && Math.abs(p.vx) > 1) ? Math.round(Math.abs(Math.sin(p.walkT)) * 1.5) : 0;
  ...
  function frame(prev, now) {
    ...
    window.__rn = { phase, a1: p1.alive, a2: p2.alive, winner: winner ? winner.name : null, camx: Math.round(cam), p1x: Math.round(p1.x), p1y: Math.round(p1.y), p1g: p1.onG };
